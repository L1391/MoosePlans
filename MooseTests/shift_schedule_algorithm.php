...
$get_pref = $db->query("SELECT users.id, p.shift_off_first, p.shift_off_second, p.shift_off_third FROM
                        users INNER JOIN teach_pref as p ON users.id = p.teach_id
                        WHERE p.wknd_id='$weekend_id' GROUP BY p.teach_id")->fetchAll();

//organize teach preferences into a matrix
$pref_matrix = [];
$teach_order = [];
foreach ($get_pref as $p) {
    $teach_pref_row = [10,10,10,10,10,10];
    $teach_pref_row[$p['shift_off_first']] = 1;
    $teach_pref_row[$p['shift_off_second']] = 2;
    $teach_pref_row[$p['shift_off_third']] = 3;
...
if (sizeof($taken_columns) < 6) {
    foreach ($nonnovel_rows as $index=>$n) {
        if (!in_array(array_search(3, $organized_matrix[$index + sizeof($original_rows)]),$taken_columns)) {
...
$unable_to_assign = [];
foreach($nonnovel_rows as $index=>$n) {
    if (!in_array(array_search(1, $organized_matrix[$index + sizeof($original_rows)]), $full_shifts) && !in_array($index + sizeof($original_rows), $already_assigned)) {
        array_push($assignments, [$new_teach_order[$index + sizeof($original_rows)], array_search(1, $organized_matrix[$index+ sizeof($original_rows)])]);
        array_push($full_shifts, array_search(1, $organized_matrix[$index+ sizeof($original_rows)]));
        array_push($already_assigned, $index + sizeof($original_rows));
    }
}

if (sizeof($assignments) < sizeof($new_teach_order)) {
    foreach($nonnovel_rows as $index=>$n) {
        if (!in_array(array_search(2, $organized_matrix[$index + sizeof($original_rows)]), $full_shifts) && !in_array($index + sizeof($original_rows), $already_assigned)) {
            array_push($assignments, [$new_teach_order[$index + sizeof($original_rows)], array_search(2, $organized_matrix[$index+ sizeof($original_rows)])]);
            array_push($full_shifts, array_search(1, $organized_matrix[$index+ sizeof($original_rows)]));
            array_push($already_assigned, $index + sizeof($original_rows));
        }
...
if (sizeof($assignments) < sizeof($new_teach_order)) {
    foreach($nonnovel_rows as $index=>$n) {
        if (!in_array(array_search(3, $organized_matrix[$index + sizeof($original_rows)]), $full_shifts) && !in_array($index + sizeof($original_rows), $already_assigned)) {
            array_push($assignments, [$new_teach_order[$index + sizeof($original_rows)], array_search(3, $organized_matrix[$index+ sizeof($original_rows)])]);
            array_push($full_shifts, array_search(1, $organized_matrix[$index+ sizeof($original_rows)]));
            array_push($already_assigned, $index + sizeof($original_rows));
        } else if (in_array(array_search(3, $organized_matrix[$index + sizeof($original_rows)]), $full_shifts) && !in_array($index + sizeof($original_rows), $already_assigned)) {
            array_push($unable_to_assign, $new_teach_order[$index + sizeof($original_rows)]);
        }
    }
}
...
foreach ($unable_to_assign as $u) {
    echo "Unable to assign teacher " . $u;
}
